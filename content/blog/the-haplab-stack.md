...
workflows, while staying with a reasonable price point.  So, for these
...
collections of "plugins".  The one I found most useful was the one that manages
...
TD knocked it out of the park! TD trys to solve the problem of coordinating the
...
team. On occasion, I used a separate claude-code instance to talk though some
ideas for feature creation. I also tried to get the free version of
[amp](https://ampcode.com/) in on this shanagans, but it kept spitting out
errors and so I gave up on that.  With respect to the claude agents, I tended
to find that the suggestion of restarting each claude agent for each new
feature worked better than letting them run for a while, but I was suprised to
find how resistent it was to actually following the content in the AGENTS.md
file.  In fact, it even deleted the file at one point.
...
WRT managing all of these parallel tasks, while I was hoping that
...
Wizard of Oz and more of a "playground" feel.

Oh, yeah, and if for some reason you are interested in a HIIT based code typing
